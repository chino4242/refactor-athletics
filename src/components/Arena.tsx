...
import { Swords, History, Users, Trophy } from 'lucide-react';
        ...
        <div className="w-full max-w-4xl mx-auto animate-fade-in-up space-y-8 p-4 md:p-0">
            ...
            {isClassic && (
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-white tracking-tight">Social</h1>
                            <p className="text-sm text-zinc-500">Challenge friends and track progress together</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                <div className="flex items-center gap-4 bg-zinc-900/50 p-1 rounded-xl border border-zinc-800">
                    ...
                    >
                        {isClassic ? <Trophy size={14} /> : <Swords size={14} />}
                        {isClassic ? 'Active Challenges' : 'Active Battles'}
                    </button>
                    <button
                ...
                <button
                    ...
                    className={isClassic
                        ? "flex items-center gap-2 bg-orange-600 hover:bg-orange-500 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all"
                        : "flex items-center gap-2 bg-red-900/40 hover:bg-red-900/60 border border-red-900 text-red-200 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all shadow-[0_0_15px_rgba(220,38,38,0.3)] hover:shadow-[0_0_25px_rgba(220,38,38,0.5)]"
                    }
                >
                    {isClassic ? <Users size={16} /> : <Swords size={16} />}
                    {isClassic ? 'Challenge a Friend' : 'Create Challenge'}
                </button>
            </div>
                ...
                {view === 'active' ? (
                    activeDuels.length === 0 ? (
                        <div className="p-12 border-2 border-dashed border-zinc-800 rounded-2xl text-center">
                            ...
                            <p className="text-zinc-500 font-bold uppercase tracking-wide">{isClassic ? 'No Active Challenges' : 'No Active Battles'}</p>
                            <p className="text-zinc-600 text-xs mt-1 max-w-xs mx-auto">
                                {isClassic
                                    ? 'Challenge a friend to see how you stack up against each other.'
                                    : 'The arena is empty. Challenge a rival to a duel and prove your strength.'}
                            </p>
                            <button
                                onClick={() => setIsChallengeModalOpen(true)}
                                className={`mt-6 text-xs underline underline-offset-4 ${isClassic ? 'text-orange-400 hover:text-orange-300' : 'text-red-400 hover:text-red-300'}`}
                            >
                                {isClassic ? 'Start a Challenge' : 'Initiate a Duel'}
                            </button>
                        </div>
                    ) : (
                        activeDuels.map(d => (
