        ...
        <div className={`p-6 rounded-2xl border transition-all shadow-lg relative overflow-hidden group ${isAlive
            ...
            <div className="flex justify-between items-start mb-6 relative z-10">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        {isAlive && <span className="text-xs bg-orange-500/20 text-orange-400 px-1.5 py-0.5 rounded border border-orange-500/30 animate-pulse">LIVE</span>}
                        {!isAlive && <span className="text-xs bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded border border-red-500/30">RESET</span>}
                    </div>
                    <h3 className="text-2xl font-black italic text-white tracking-tighter">{challenge.name}</h3>
                    <p className="text-base text-zinc-400 font-medium">Day <span className="text-white">{currentDay}</span> of {challenge.duration_days}</p>
                </div>
                ...
                <div className="flex flex-col items-end">
                    ...
                    <span className="text-xs font-bold text-zinc-600 uppercase tracking-widest">Day Streak</span>
                </div>
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-white/5 flex justify-between items-center relative z-10">
                <div className="flex items-center gap-4">
                    ...
                    {isAlive ? (
                        <button
                            ...
                            className="text-xs uppercase font-bold text-zinc-600 hover:text-red-500 flex items-center gap-1 transition-colors"
                        ...
                        <button
                            onClick={handleCancel}
                            className="text-xs uppercase font-bold text-red-500 hover:text-red-400 flex items-center gap-1 transition-colors animate-pulse"
                ...
                <div className="flex items-center gap-2">
                    {checking && <span className="text-sm text-orange-500 animate-pulse">Syncing...</span>}
                    {!checking && !needsCheck && (
