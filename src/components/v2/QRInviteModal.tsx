  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
      ...
      <div className="relative text-center space-y-4 p-8" onClick={e => e.stopPropagation()}>
        {partyName && (
          <p className="text-base text-white font-medium" style={{ fontFamily: "var(--font-pixel), monospace" }}>{partyName}</p>
        )}
        {memberCount !== undefined && (
          <p className="text-xs text-zinc-400">{memberCount} member{memberCount !== 1 ? 's' : ''}</p>
        )}
        <div className="bg-white p-4 rounded-lg inline-block">
          <canvas ref={canvasRef} width={200} height={200} />
        </div>
        <p className="text-xs text-zinc-500 break-all max-w-[250px] mx-auto">{url}</p>
        <p className="text-xs text-zinc-600 mt-4">Scan to join · tap anywhere to close</p>
      </div>
    </div>
  );
}
