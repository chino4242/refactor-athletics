      ...
      <div className="space-y-1.5">
        {members.map((m, i) => (
          <div key={m.userId} className={`flex items-center justify-between px-2 py-1 ${m.isYou ? `border ${colors.border} bg-zinc-800/50` : ''}`}>
            ...
            <div className="flex items-center gap-2">
              <span className={`text-[9px] ${m.isYou ? colors.secondary : 'text-zinc-300'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>PL {m.powerLevel}</span>
              {m.weekDelta !== 0 && (
                <span className={`text-[7px] ${m.weekDelta > 0 ? 'text-green-400' : 'text-red-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>
