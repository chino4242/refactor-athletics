      ...
      <div className={`w-full max-w-sm border-2 ${colors.primary} bg-zinc-900 relative max-h-[85vh] flex flex-col`}>
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1">
          {step === 1 && (
            <>
              <div>
                <p className="text-[8px] text-zinc-500 uppercase mb-2" style={{ fontFamily: "var(--font-pixel), monospace" }}>NAME</p>
                <input
                  value={name}
                ...
                <div className="flex gap-2">
                  {DURATION_PRESETS.map(d => (
                    <button key={d} onClick={() => setDuration(d)} className={`flex-1 py-2 border text-center ${duration === d ? `${colors.primary} bg-zinc-800` : 'border-zinc-700 bg-zinc-900'}`}>
                      <span className={`text-[9px] ${duration === d ? colors.secondary : 'text-zinc-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>{d}D</span>
                    </button>
                  ))}
                  <input
                    type="number"
                    ...
                    className={`w-16 bg-zinc-800 border ${colors.border} px-2 py-2 text-center text-white text-[9px]`}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setScope('solo')} className={`flex-1 py-2 border ${scope === 'solo' ? `${colors.primary} bg-zinc-800` : 'border-zinc-700 bg-zinc-900'}`}>
                    <span className={`text-[9px] ${scope === 'solo' ? colors.secondary : 'text-zinc-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>SOLO</span>
                  </button>
                  <button onClick={() => setScope('group')} disabled={!groupId} className={`flex-1 py-2 border ${scope === 'group' ? `${colors.primary} bg-zinc-800` : 'border-zinc-700 bg-zinc-900'} disabled:opacity-40`}>
                    <span className={`text-[9px] ${scope === 'group' ? colors.secondary : 'text-zinc-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>GROUP</span>
                  </button>
                </div>
              </div>
              {scope === 'group' && (
                <div>
                  <p className="text-[8px] text-zinc-500 uppercase mb-2" style={{ fontFamily: "var(--font-pixel), monospace" }}>FAILURE MODE</p>
                  <div className="flex gap-2">
                    <button onClick={() => setSharedFate(false)} className={`flex-1 py-2 border ${!sharedFate ? `${colors.primary} bg-zinc-800` : 'border-zinc-700 bg-zinc-900'}`}>
                      <span className={`text-[8px] ${!sharedFate ? colors.secondary : 'text-zinc-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>INDIVIDUAL</span>
                    </button>
                    <button onClick={() => setSharedFate(true)} className={`flex-1 py-2 border ${sharedFate ? 'border-red-700 bg-zinc-800' : 'border-zinc-700 bg-zinc-900'}`}>
                      <span className={`text-[8px] ${sharedFate ? 'text-red-400' : 'text-zinc-400'}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>SHARED FATE</span>
                    </button>
                  </div>
                  {sharedFate && <p className="text-[7px] text-red-500 mt-1" style={{ fontFamily: "var(--font-pixel), monospace" }}>⚠ ONE FAILS = ALL FAIL</p>}
                </div>
              )}
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div>
                <p className="text-[8px] text-zinc-500 uppercase mb-2" style={{ fontFamily: "var(--font-pixel), monospace" }}>APP METRICS (AUTO-CHECK)</p>
                <div className="space-y-1 mb-2">
                  {appMetrics.map(m => (
                    <div key={m.id} className="flex items-center justify-between border border-zinc-700 bg-zinc-800 px-2 py-1.5">
                      <span className="text-[8px] text-zinc-200" style={{ fontFamily: "var(--font-pixel), monospace" }}>{m.label}</span>
                      <div className="flex items-center gap-1">
                        {m.id !== 'workout_count' && (
                          <input
                            ...
                            className="w-14 bg-zinc-900 border border-zinc-600 px-1 py-0.5 text-[8px] text-white text-center"
                            style={{ fontFamily: "var(--font-pixel), monospace" }}
                          />
                        )}
                ...
                <div className="flex flex-wrap gap-1">
                  {APP_METRICS.filter(m => !appMetrics.find(a => a.id === m.id)).map(m => (
                    <button key={m.id} onClick={() => addAppMetric(m)} className="text-[7px] px-2 py-1 border border-zinc-700 bg-zinc-900 text-zinc-400 hover:text-white" style={{ fontFamily: "var(--font-pixel), monospace" }}>
                      + {m.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <p className="text-[8px] text-zinc-500 uppercase mb-2" style={{ fontFamily: "var(--font-pixel), monospace" }}>MANUAL HABITS (HONOR SYSTEM)</p>
                <div className="space-y-1 mb-2">
                  {customItems.map((item, i) => (
                    <div key={i} className="flex items-center justify-between border border-zinc-700 bg-zinc-800 px-2 py-1.5">
                      <span className="text-[8px] text-zinc-200" style={{ fontFamily: "var(--font-pixel), monospace" }}>{item}</span>
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    className={`flex-1 bg-zinc-800 border ${colors.border} px-2 py-1.5 text-[8px] text-white`}
                    style={{ fontFamily: "var(--font-pixel), monospace" }}
                  />
                  <button onClick={addCustomItem} className={`px-2 border ${colors.border} bg-zinc-800 text-zinc-400 hover:text-white text-[8px]`} style={{ fontFamily: "var(--font-pixel), monospace" }}>+</button>
                </div>
              </div>
            </>
          )}
          ...
          {step === 3 && (
            <div className="space-y-3">
              <p className={`text-[9px] ${colors.secondary} text-center`} style={{ fontFamily: "var(--font-pixel), monospace" }}>CONFIRM YOUR OATH</p>
              <div className="border border-zinc-700 bg-zinc-800 p-3 space-y-1">
                <p className="text-xs text-white">{name}</p>
                <p className="text-[8px] text-zinc-500" style={{ fontFamily: "var(--font-pixel), monospace" }}>{duration} DAYS · {scope.toUpperCase()}{sharedFate ? ' · SHARED FATE' : ''}</p>
              </div>
              <div className="space-y-1">
                {appMetrics.map(m => (
                  <div key={m.id} className="flex items-center gap-2 text-[8px]" style={{ fontFamily: "var(--font-pixel), monospace" }}>
                ...
                {customItems.map((item, i) => (
                  <div key={i} className="flex items-center gap-2 text-[8px]" style={{ fontFamily: "var(--font-pixel), monospace" }}>
              ...
              <p className="text-[7px] text-zinc-600 text-center" style={{ fontFamily: "var(--font-pixel), monospace" }}>STARTS TOMORROW · REWARD: 2,500 XP</p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-zinc-800 flex gap-2 shrink-0">
          ...
          {step < 3 ? (
            <button onClick={() => setStep(step + 1)} disabled={step === 2 && appMetrics.length === 0 && customItems.length === 0} className={`flex-1 py-3 border-2 ${colors.primary} bg-zinc-800 text-white disabled:opacity-40`} style={{ fontFamily: "var(--font-pixel), monospace" }}>
              <span className={`text-[9px] ${colors.secondary}`}>NEXT ▸</span>
            </button>
          ) : (
            <button onClick={handleCreate} disabled={loading} className={`flex-1 py-3 border-2 ${colors.primary} bg-zinc-800 text-white disabled:opacity-40`} style={{ fontFamily: "var(--font-pixel), monospace" }}>
              <span className={`text-[9px] ${colors.secondary}`}>{loading ? 'FORGING...' : '⚔ BEGIN CAMPAIGN'}</span>
            </button>
          )}
        </div>
      </div>
