    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center" onClick={onDismiss}>
      ...
      <div className="relative text-center space-y-5 px-8 max-w-sm w-full animate-in fade-in zoom-in duration-500" onClick={e => e.stopPropagation()}>
        ...
        <div>
          ...
          {data.streakPhase && (
            <p className="text-[9px] text-amber-400 mt-1" style={{ fontFamily: "var(--font-pixel), monospace" }}>
        ...
        <div className="grid grid-cols-2 gap-3 text-left">
          {data.exercises > 0 && (
            <div className={`border ${colors.border} bg-zinc-900 p-3`}>
              <p className="text-[8px] text-zinc-500 uppercase" style={{ fontFamily: "var(--font-pixel), monospace" }}>Exercises</p>
              <p className="text-lg text-white" style={{ fontFamily: "var(--font-pixel), monospace" }}>{data.exercises}</p>
            </div>
          )}
          {data.rankUps > 0 && (
            <div className={`border ${colors.border} bg-zinc-900 p-3`}>
              <p className="text-[8px] text-zinc-500 uppercase" style={{ fontFamily: "var(--font-pixel), monospace" }}>Rank-Ups</p>
              <p className="text-lg text-amber-400" style={{ fontFamily: "var(--font-pixel), monospace" }}>{data.rankUps}</p>
            </div>
          )}
          {data.steps > 0 && (
            <div className={`border ${colors.border} bg-zinc-900 p-3`}>
              <p className="text-[8px] text-zinc-500 uppercase" style={{ fontFamily: "var(--font-pixel), monospace" }}>Steps</p>
          ...
          {data.bountiesTotal > 0 && (
            <div className={`border ${colors.border} bg-zinc-900 p-3`}>
              <p className="text-[8px] text-zinc-500 uppercase" style={{ fontFamily: "var(--font-pixel), monospace" }}>Bounties</p>
        ...
        <button onClick={onDismiss} className={`text-[10px] px-6 py-3 border ${colors.primary} bg-zinc-800 ${colors.secondary}`} style={{ fontFamily: "var(--font-pixel), monospace" }}>
          BEGIN TODAY
        </button>
      </div>
    </div>
