  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md animate-fade-in" onClick={() => setLevelUp(null)}>
      ...
      <div className="relative text-center px-8 max-w-sm">
        ...
        {reward?.lore_text && (
          <p className="text-base text-zinc-400 italic mb-6">"{reward.lore_text}"</p>
        )}
        ...
        {reward?.unlock_label && !reward.version_gate && (
          <div className="bg-orange-500/10 border border-orange-500/30 rounded-xl px-4 py-3 mb-6">
            <div className="text-xs text-orange-400 uppercase tracking-wider font-bold mb-1">New Unlock</div>
            <div className="text-base font-bold text-white">{reward.unlock_label}</div>
          </div>
        )}

        {reward?.unlock_label && reward.version_gate && (
          <div className="bg-zinc-800/50 border border-zinc-700 rounded-xl px-4 py-3 mb-6">
            <div className="text-xs text-zinc-500 uppercase tracking-wider font-bold mb-1">Coming in {reward.version_gate.toUpperCase()}</div>
            <div className="text-base font-medium text-zinc-400">🔒 {reward.unlock_label}</div>
          </div>
        )}

        <div className="text-xs text-zinc-600 uppercase tracking-wider animate-pulse">Tap to continue</div>
      </div>
    </div>
  );
}
